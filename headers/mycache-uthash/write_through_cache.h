...
void write_through_cache(struct MY_DB *db, void *block, int block_id) {
	...
	HASH_FIND_INT(global_hash, &block_id, s);
	if(s != NULL) {
		current = s->cache_block_ptr;
	...
	*/
	...
	if(db->cache.occuped_blocks == db->cache.total_blocks) {
		...
		HASH_FIND_INT(global_hash, &(db->cache.last->block_id), s);
		HASH_DEL(global_hash, s);
		...
		s->id = block_id;
		s->cache_block_ptr = current;
		HASH_ADD_INT( global_hash, id, s );
